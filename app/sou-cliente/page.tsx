...
export default function ClientPage() {
  ...
  const handleGoogleLogin = () => {
    try {
      ...
      window.location.href = 'https://barberflow-back-end-19nv.onrender.com/api/client/auth/google';
    } catch (error) {
  ...
  const handleFacebookLogin = () => {
    try {
      setOauthLoading(true);
      window.location.href = 'https://barberflow-back-end-19nv.onrender.com/api/client/auth/facebook';
    } catch (error) {
    ...
    try {
      ...
      if (userLocation) {
        const response = await fetch(
          `https://barberflow-back-end-19nv.onrender.com/api/barbershop-location/nearby?lat=${userLocation.lat}&lng=${userLocation.lng}&radius=${radius}`
        );
        const data = await response.json();
        setBarbershops(data.barbershops || []);
      } else {
        // Buscar todas (modo antigo)
        const url = 'https://barberflow-back-end-19nv.onrender.com/api/public/barbershops';
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch('https://barberflow-back-end-19nv.onrender.com/api/client/auth/login', {
        method: 'POST',
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://barberflow-back-end-19nv.onrender.com/api/client/auth/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const openBarbershopDetails = async (barbershop: Barbershop) => {
    try {
      const response = await fetch(`https://barberflow-back-end-19nv.onrender.com/api/public/barbershops/${barbershop.id}`);
      const data = await response.json();
  ...
  useEffect(() => {
    const loadAvailableTimes = async () => {
      ...
      try {
        const response = await fetch(
          `https://barberflow-back-end-19nv.onrender.com/api/public/barbershops/${selectedBarbershop.id}/available-times?date=${selectedDate}&serviceId=${selectedService}&barberId=${selectedBarber}`
        );
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setLoading(false);
          // ✅ NÃO ABRE O MAPA AUTOMATICAMENTE (removido setShowMap(true))
  ...
  const handleCepSearch = async () => {
    ...
    try {
      const response = await fetch(
        `https://barberflow-back-end-19nv.onrender.com/api/barbershop-location/geocode/cep/${cleanCep}`
      );
      ...
      <header className="border-b border-gray-800 px-4 py-3 bg-black">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={toggleTheme}
              className="hidden md:flex items-center gap-2 text-gray-300 hover:text-white transition text-sm p-2 rounded-lg hover:bg-gray-800"
            ...
            <div className="p-8">
              ...
              {authMode === 'login' ? (
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                  ...
                  {/* ✅ CHECKBOX DE TERMOS - ADICIONAR ANTES DO BOTÃO */}
                  <div className="mb-4">
              ...
              <p className="text-center text-xs text-gray-500 mt-4">
                ...
                <a href="/termos-cliente" className="text-[#2463eb] hover:text-[#1d4fd8]">termo de uso</a>
              </p>
            </div>
          </div>
        </div>
